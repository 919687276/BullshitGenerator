<template>
  <div class="body">
    <div class="top">
      <div class="title">好喜欢思思呀</div>
      <div class="main1">
        <div>主题</div>
        <div class="input1"><el-input v-model="title" placeholder="请输入主题" size="mini"></el-input></div>
      </div>
      <div class="main2">
        <div>字数</div>
        <div class="input1"><el-input v-model="number" placeholder="请输入字数" size="mini"></el-input></div>
        <el-button type="info" round @click="article()"
          size="mini" class="btn">生成</el-button>
      </div>
    </div>
...
export default {
  data(){
    return{
      ...
      number: '',
      famousSayings:[
        "爱迪生曾经说过，天才是百分之一的勤奋加百分之九十九的汗水。这不禁令我深思",
        "查尔斯·史曾经说过，一个人几乎可以在任何他怀有无限热忱的事情上成功。这不禁令我深思",
        "培根曾经说过，深窥自己的心，而后发觉一切的奇迹在你自己。这不禁令我深思",
        "歌德曾经说过，流水在碰到底处时才会释放活力。这不禁令我深思",
        "莎士比亚曾经说过，那脑袋里的智慧，就像打火石里的火花一样，不去打它是不肯出来的。这不禁令我深思",
        "戴尔·卡耐基曾经说过，多数人都拥有自己不了解的能力和机会，都有可能做到未曾梦想的事情。这不禁令我深思",
      ...
      discuss:[ 
        ...
        "主题似乎是一种巧合，但如果我们从一个更大的角度看待问题，这似乎是一种不可避免的事实. ",
  ...
  methods:{
    article(){
      ...
      for(let i in this.title){
          let chapter = "";// 章节
          let chapterLength = 0;// 章节长度
          while( chapterLength < this.number ){
              let random = this.randMath(0,100);// 获取一个随机数
              if(random < 5 && chapter.length > 200){
                  chapter = this.addParagraph(chapter);
                  articleContant.push(chapter); 
                  chapter = "";
              }else if(random < 20){
                  let sentence = this.createFamousSayings();
                  chapterLength = chapterLength + sentence.length;
                  chapter = chapter + sentence;
              }else{
                  let sentence = this.moreDiscuss();
                  chapterLength = chapterLength + sentence.length;
                  chapter = chapter + sentence;
              }
          }
      ...
      let Typesetting = "<div>" + articleContant.join("</div><div>") + "</div>";
      document.getElementById('contant').innerHTML = Typesetting;
    },
    randMath(min,max){
    ...
    createFamousSayings(){
      let wellKnownSaying = this.TakeAandomSentence(this.famousSayings)
      wellKnownSaying = wellKnownSaying.replace("曾经说过", this.TakeAandomSentence(this.frontIntroduction) )
      wellKnownSaying = wellKnownSaying.replace("这不禁令我深思", this.TakeAandomSentence(this.backIntroduction) )
      return wellKnownSaying
    },
    TakeAandomSentence(list){
      let coordinate = Math.floor( Math.random() * list.length );
      return list[coordinate];
    },
    moreDiscuss(){
      let sentence = this.TakeAandomSentence(this.discuss);
      sentence = sentence.replace(RegExp('主题', "g"),this.title);
      return sentence;
    }
  }
}
